...
## Data preparation and sample check
...
distribution. It's supposed to be sigma squared over N.

```{r}
(means.variance <- var(means))
(theoretical.variance <- 1/(sample.size*lambda^2))
DifferenceWithTheory(means.variance, theoretical.variance)
```
...
```{r}
breaks <- 25
# What share of 1000 simulations will fit to the central highest bar?
pnorm(
hist.height <- numsim * diff(qnorm(c(-1/breaks/2,1/breaks/2)))

hist.param <- hist(means, breaks=breaks)
...
## Appendix 2. Overlay curve height

We used 25 bars histogram. What should be its height for normal distribution?
First, let's check it via simulation.
```{r}
larger <- 1000
round(max(hist(rnorm(numsim * larger), breaks=breaks)$counts)/larger)
```

OK, it's about 192, but how do we calculate it theoretically? It probably
depends on how hist() function works - how few cases it consider still worthy
of a separate bar on the chart. Suppose all bar-worth values are with 99%
range. Then with 25 bars we should have 25 half-bars on each side of the middle.
```{r}
where.bars.end <- qnorm(0.995)
halfbar <- where.bars.end/(2*breaks/2)
numsim*(pnorm(halfbar)-pnorm(-halfbar))
```

Suppose we need to count how many cases is in the central bar. Suppose bars will
end at 99% of probability.
This means - what is probability that an observation has value of 1-